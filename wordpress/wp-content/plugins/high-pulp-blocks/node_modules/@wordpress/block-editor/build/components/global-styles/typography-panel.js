...
function useHasFontSizeControl(settings) {
  return (0, _getBlockSettings.hasMergedOrigins)(settings?.typography?.fontSizes) || settings?.typography?.customFontSize;
}
function useHasFontFamilyControl(settings) {
  return (0, _getBlockSettings.hasMergedOrigins)(settings?.typography?.fontFamilies);
}
...
function getUniqueFontSizesBySlug(settings) {
  ...
  const mergedFontSizes = fontSizes ? (0, _getBlockSettings.mergeOrigins)(fontSizes) : [];
  const uniqueSizes = [];
